...
%   maxChunkElements = target maximum elements per shuffle chunk (default: 5e8)
...
function analyzeSingleSession(sessionTable, cellColumns, outputDir, outputStem, opts)
    ...
    %% shuffle calcium traces and compute signal peaks in chunks
    chunkSize = max(1, floor(opts.maxChunkElements / max(1, numFrames * numNeurons)));
    chunkSize = min(chunkSize, opts.numShuffles);

    allPeaksShuffledPath = fullfile(outputDir, [outputStem '_allPeaksShuffled.h5']);
    recreateH5(allPeaksShuffledPath);
    h5create(allPeaksShuffledPath, '/allPeaksShuffled', [numFrames, numNeurons, Inf], ...
        'Datatype', 'double', 'ChunkSize', [numFrames, numNeurons, chunkSize]);
    ...
    for startShuffle = 1:chunkSize:opts.numShuffles
        ...
        for shuffle = 1:currentChunkSize
            disp(['Shuffle ' num2str(startShuffle + shuffle - 1) ' for ' outputStem]);
            ...
            signalPeaksThisShuffle = computeSignalPeaks(shuffledCellTraces', 'doMovAvg', 0, 'reportMidpoint', 1, 'numStdsForThresh', opts.numStdsForThresh)';
            allPeaksShuffledChunk(:, :, shuffle) = signalPeaksThisShuffle;
        end

        count = [numFrames, numNeurons, currentChunkSize];
        h5write(allPeaksShuffledPath, '/allPeaksShuffled', allPeaksShuffledChunk, [1, 1, startShuffle], count);
    end
    ...
    for startShuffle = 1:chunkSize:opts.numShuffles
        ...
        allPeaksShuffledChunk = h5read(allPeaksShuffledPath, '/allPeaksShuffled', ...
        ...
        for shuffleIdx = 1:currentChunkSize
            ...
            disp(['Calculating mutual information for shuffle: ' num2str(shuffle) ' for ' outputStem]);
